...
nbr_erreur  = 100;  % Nombre d'erreurs Ã  observer avant de calculer un BER
nbr_bit_max = 100e6;% Nombre de bits max Ã  simuler
ber_min     = 3e-5; % BER min

EbN0dB = EbN0dB_min:EbN0dB_step:EbN0dB_max;     % Points de EbN0 en dB Ã  simuler
EbN0   = 10.^(EbN0dB/10);% Points de EbN0 Ã  simuler
EsN0   = R*log2(M)*EbN0; % Points de EsN0
EsN0dB = 10*log10(EsN0); % Points de EsN0 en dB Ã  simuler
...
p3 = [013 015];
p4 = [133 171];
print_tab_recap([p1; p2 ; p3; p4],[2 3 4 7],K);

%% Préparation de l'affichage
figure(1)
h_ber = [];
for i=(1:5)
    h_ber(i) = semilogy(EbN0dB,ber,'XDataSource','EbN0dB', 'YDataSource','ber');
end
hold all
ylim([1e-6 1])
grid on
...
figure(1)
semilogy(EbN0dB,ber);
legend('(133, 171)_{8}', 'TEB (non cod\''e)', '(7, 5)', '(2, 3)_{8}','(13, 15)');
hold all
xlim([-2 13])
ylim([1e-6 1])
grid on
